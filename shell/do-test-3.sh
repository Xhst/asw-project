...
curl -X POST "http://localhost:8080/order-service/orders" -H "accept: */*" -H "Content-Type: application/json" \
...
echo $(curl -s localhost:8080/order-validation-service/ordervalidations/7) | json_pp
echo 

# non deve dare errore, ma non deve passare la validazione (l'ordine non esiste) 

echo "# convalida l'ordine 99" 
echo $(curl -s localhost:8080/order-validation-service/ordervalidations/99) | json_pp
echo 